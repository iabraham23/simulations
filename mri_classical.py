...
Mvec_label=label(pos=Mvec.pos+1.1*Mvec.axis+vector(-100,0,0), text='Equilibrium\n    Magnetization', color=Mvec.color, box=False, height=1.1*text_size, opacity=0)
...
timescale_label=label(pos=vector(mri_signal_label.pos.x+55,-25,0), text="0    time (after RF pulse)     1000 (ms)", height = text_size, box=False, opacity=0, color=color.black, visible=True)
...
csfgraphlabel1=label(pos=vector(-450,40,0), text='CS Fluid', height=text_size, box=False, opacity=0, align='left', align='left', color=color.magenta, visible=False)
...
#Functions
def reset():
    global isRunning, isStarted, B_mag, B_dir, B,B_vec,  M_dir, M, M_tip, Mvec, M_mag, M_dir, t, torque_vec, torque_label, M_vertical_vec, M_horizontal_vec, voltage_list, voltage_graph, dt, mindex, recoveredMz_list
    isRunning = False
    isStarted = False
    B_mag = 1
    ...
    Bvec = arrow(pos=-vec(-1,0,0), axis=pixel_D*mag_h*B, shaftwidth=7.5*w_scale*(mag_h/100), headwidth=15*w_scale*(mag_h/100), headlength=30*w_scale*(mag_h/100), color=norm(vec(255,103,35)), round=True, visible=False)
    torque_vec = arrow(pos=Mvec.pos+Mvec.axis, axis=pixel_D*mag_h/1.5*norm(cross(M,B)), shaftwidth=5*w_scale*(mag_h/100), headwidth=10*w_scale*(mag_h/100), headlength=20*w_scale*(mag_h/100), color=norm(vec(0,210,106)), round=True, visible=False)
    torque_label = label(pos=torque_vec.pos + torque_vec.axis + vector(2*text_size,0,0), text='τ<sub>RF</sub>', height=1.3*text_size, box=False, color=color.green, opacity=0, visible=False)
    #Hidden under Mvec
...
create_buttons(control_panel, ['Play/Pause', 'Reset'], ['⏯','↻'])
...
while True:
    ...
    while isRunning and t>=t_final_1 and t<t_final_3:
        ...
        if isRunning: #redundant?
            ...
            M_mag = exp(-(t-t_final_1)/T2)
            M=M_mag*M_dir
            Mxy_label.visible=True
            mri_signal_label.visible=True
            M_vertical_vec.color=vec(0.7,0,0.9)
            M_vertical_vec.opacity=1
            M_horizontal_vec.axis=M_horizontal_vec_after_pulse*exp(-(t-t_final_1)/T2)
            voltage=vector(1000*t-300,170*exp(-(t-t_final_1)/T2),0)
            voltage_list.append(voltage)
            voltage_graph=curve(canvas=scene, pos=voltage_list, radius=2, color=color.blue, origin=vector(-975,0,0))
            M_horizontal_vec.headwidth = starting_headw*mag(M_horizontal_vec.axis)/mag_h*.9
            M_horizontal_vec.headlength = starting_headl*mag(M_horizontal_vec.axis)/mag_h*.9
            M_horizontal_vec.shaftwidth = starting_shaftw*mag(M_horizontal_vec.axis)/mag_h*.9
            ...
            M_vertical_vec.axis = pixel_D*mag_h*M*(1-exp(-(t-t_final_1)/T1))+vector(0,starting_headl*exp(-(t-t_final_1)/T1),0) 
            ...
            recoveredMz_list.append(vector(1000*t-300,170*(1-exp(-(t-t_final_1)/T1)),0))
            recoveredMz_graph=voltage_graph=curve(canvas=scene, pos=recoveredMz_list, radius=2, color=vec(0.9,0,0.7), origin=voltage_graph.origin)
